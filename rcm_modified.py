...
def get_config_path():
    # Determine the base path based on execution mode
    if getattr(sys, 'frozen', False):  # Running as a bundled app
        base_path = os.path.join(sys._MEIPASS, "Resources")  # Use Resources inside .app
    else:
...
class RelayControllerApp:
    def __init__(self, root):
        ...
        self.board_selector = ttk.Combobox(root, textvariable=self.num_boards, values=[1, 2, 3, 4, 5, 6], state="readonly")
        ...
        self.board_buttons_frame = tk.Frame(root)
        self.board_buttons_frame.grid(row=5, column=2, rowspan=2, sticky="ns")
        self.board_fire_buttons = []
        for i in range(6):
            btn = tk.Button(self.board_buttons_frame, text=f"Fire Board {i+1}", command=lambda i=i: self.fire_board(i+1))
            btn.grid(row=i, column=0, padx=2, pady=2)
            self.board_fire_buttons.append(btn)
    ...
    def fire_board(self, board_num):
        start = (board_num - 1) * RELAYS_PER_BOARD + 1
        end = start + RELAYS_PER_BOARD
        command = "".join([f"S{relay:02d}%" for relay in range(start, end)])
        self.send_command(command)
        for relay in range(start, end):
            if relay in self.relay_states:
                self.relay_states[relay].set(True)
